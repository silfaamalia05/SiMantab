...
@section('content')
    <div class="container mx-auto p-6">
        <h2 class="text-xl font-semibold mb-4 dark:text-white">{{ isset($data) ? 'Edit' : 'Tambah' }} Data Batas Kota / Kab</h2>
        <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-4">
            <div class="flex justify-between text-center items-center dark:text-white my-2">
                <h6>{{ isset($data) ? 'Edit' : 'Tambah' }} Data Batas Kota / Kab</h6>
                <a href="{{ route('batas_kab_kota.index') }}" class="bg-blue-900 text-white px-6 py-2 no-underline"><i
            ...
            @endif
            <form action="{{ isset($data) ? route('batas_kab_kota.update', $data->id) : route('batas_kab_kota.store') }}" method="POST" enctype="multipart/form-data">
                @method(isset($data) ? 'PUT' : 'POST')
                @csrf
                <div>
                    <label for="KDPKAB"
                        class="block text-sm font-medium text-gray-900 dark:text-white mt-2 mb-1">Kode Kab / Kota</label>
                    <input type="text" name="KDPKAB" id="KDPKAB" required
                        placeholder="Kode Kab / Kota"
                        class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        value="{{ isset($data) ? $data->KDPKAB : old('KDPKAB') }}"
                        {{ !isset($data) ? 'required' : '' }}>
                </div>
                <div>
                    <label for="WADMKK"
                        class="block text-sm font-medium text-gray-900 dark:text-white mt-2 mb-1">Nama Kota</label>
                    <input type="text" name="WADMKK" id="WADMKK" required
                        placeholder="Nama Kota"
                        class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        value="{{ isset($data) ? $data->WADMKK : old('WADMKK') }}"
                        {{ !isset($data) ? 'required' : '' }}>
                </div>
                <div>
