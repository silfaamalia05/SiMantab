...
@section('content')
    <div class="container mx-auto p-6">
        <h2 class="text-xl font-semibold mb-4 dark:text-white">Data Potensi Kerugian</h2>
        <div class="bg-white shadow-md rounded-lg p-4">
            <div class="flex justify-between text-center items-center my-2">
                <h6>Data Potensi Kerugian</h6>
            </div>
            ...
            <table class="min-w-full bg-white border border-gray-200 rounded-lg">
                <thead>
                    <tr class="bg-gray-100">
                        ...
                        <th class="py-2 px-4 border-b text-left">Kerugian Fisik</th>
                        <th class="py-2 px-4 border-b text-left">Kerugian Ekonomi</th>
                        <th class="py-2 px-4 border-b text-left">Potensi Kerusakan Lingkungan</th>
                    ...
                    @forelse ($data as $item)
                        <tr>
                            ...
                            <td class="py-2 px-4 border-b text-left">
                                {{ $item->PotensiKerugian->kerugian_fisik ?? '0' }}</td>
                            <td class="py-2 px-4 border-b text-left">
                                {{ $item->PotensiKerugian->kerugian_ekonomi ?? '0' }}</td>
                            <td class="py-2 px-4 border-b text-left">
                                {{ $item->PotensiKerugian->potensi_kerusakan_lingkungan ?? '0' }}</td>
                            <td class="py-2 px-4 border-b text-center"> <a
                                    href="{{ route('potensi_kerugian.form', $item->id) }}"
