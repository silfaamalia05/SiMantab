...
@section('content')
    <div class="container mx-auto p-6">
        <h2 class="text-xl font-semibold mb-4 dark:text-white">Set Tabulasi Resiko Banjir</h2>
        <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-4">
            <div class="flex justify-between text-center items-center dark:text-white my-2">
                <h6>Set Tabulasi Resiko Banjir</h6>
                <a href="{{ route('tabulasi_resiko') }}" class="bg-blue-900 text-white px-6 py-2 no-underline"><i
            ...
            @endif
            <form action="{{ route('tabulasi_resiko.set') }}" method="POST" enctype="multipart/form-data">
                @method('POST')
                @csrf
                ...
                <div>
                    ...
                    <input type="text" name="lb_rendah" id="lb_rendah" required placeholder="Lebar Bahaya (Rendah)"
                        ...
                        value="{{ isset($data->TabulasiResikoBanjir->lb_rendah) ? $data->TabulasiResikoBanjir->lb_rendah : '0' }}"
                        {{ !isset($data) ? 'required' : '' }}>
                </div>
                <div>
                    ...
                    <input type="text" name="lb_sedang" id="lb_sedang" required placeholder="Lebar Bahaya (Sedang)"
                        class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        value="{{ isset($data->TabulasiResikoBanjir->lb_sedang) ? $data->TabulasiResikoBanjir->lb_sedang : '0' }}"
                        {{ !isset($data) ? 'required' : '' }}>
                </div>
                <div>
                    ...
                    <input type="text" name="lb_tinggi" id="lb_tinggi" required placeholder="Lebar Bahaya (Tinggi)"
                        class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        value="{{ isset($data->TabulasiResikoBanjir->lb_tinggi) ? $data->TabulasiResikoBanjir->lb_tinggi : '0' }}"
                        {{ !isset($data) ? 'required' : '' }}>
                </div>
                <div>
                    <label for="kelas"
                    ...
                    <select name="kelas" id="kelas"
                        class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        <option value="Rendah"
                            {{ isset($data->TabulasiResikoBanjir->kelas) && $data->TabulasiResikoBanjir->kelas == 'Rendah' ? 'selected' : '' }}>
                            Rendah</option>
                        <option value="Sedang"
                            {{ isset($data->TabulasiResikoBanjir->kelas) && $data->TabulasiResikoBanjir->kelas == 'Sedang' ? 'selected' : '' }}>
                            Sedang</option>
                        <option value="Tinggi"
                            {{ isset($data->TabulasiResikoBanjir->kelas) && $data->TabulasiResikoBanjir->kelas == 'Tinggi' ? 'selected' : '' }}>
                            Tinggi</option>
                        </option>
                    </select>
                </div>
