...
@section('content')
    <div class="container mx-auto p-6">
        <h2 class="text-xl font-semibold mb-4 dark:text-white">Data Banjir</h2>
        <div class="bg-white dark:bg-gray-800 dark:text-white shadow-md rounded-lg p-4">
            <div class="flex justify-between text-center items-center my-2">
                <h6>Data Banjir</h6>
                <a href="{{ route('data_banjir.create') }}" class="bg-blue-900 text-white px-6 py-2 no-underline"><i
                ...
                <tbody>
                    @forelse ($data as $item)
                        <tr class="border-b">
                            ...
                            <th class="py-2 px-4 border-b text-left">{{ $item->kode_data }}</th>
                            <th class="py-2 px-4 border-b text-left">{{ $item->nama_data }}</th>
                            ...
                            <td class="py-2 px-4 text-center">
                                <form action="{{ route('data_banjir.destroy', $item->id) }}" method="POST">
                                    @method('DELETE')
                                    @csrf
                                    <a href="{{ route('data_banjir.edit', $item->id) }}"
