...
@section('content')
    <div class="container mx-auto p-6">
        <h2 class="text-xl font-semibold mb-4 dark:text-white">{{ isset($data) ? 'Edit' : 'Tambah' }} Data Kawasan Pemukiman</h2>
        <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-4">
            <div class="flex justify-between text-center items-center dark:text-white my-2">
                <h6>{{ isset($data) ? 'Edit' : 'Tambah' }} Data Kawasan Pemukiman</h6>
                <a href="{{ route('kawasan_pemukiman.index') }}" class="bg-blue-900 text-white px-6 py-2 no-underline"><i
            ...
            @endif
            <form action="{{ isset($data) ? route('kawasan_pemukiman.update', $data->id) : route('kawasan_pemukiman.store') }}" method="POST" enctype="multipart/form-data">
                @method(isset($data) ? 'PUT' : 'POST')
                @csrf
                <div>
                    <label for="kode_kawasan_pemukiman"
                        class="block text-sm font-medium text-gray-900 dark:text-white mt-2 mb-1">Kode Kawasan Pemukiman</label>
                    <input type="text" name="kode_kawasan_pemukiman" id="kode_kawasan_pemukiman" required
                        placeholder="Kode Kawasan Pemukiman"
                        class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        value="{{ isset($data) ? $data->kode_kawasan_pemukiman : old('kode_kawasan_pemukiman') }}"
                        {{ !isset($data) ? 'required' : '' }}>
                </div>
                <div>
                    <label for="nama_kawasan_pemukiman"
                        class="block text-sm font-medium text-gray-900 dark:text-white mt-2 mb-1">Nama Kota</label>
                    <input type="text" name="nama_kawasan_pemukiman" id="nama_kawasan_pemukiman" required
                        placeholder="Nama Kawasan Pemukiman"
                        class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        value="{{ isset($data) ? $data->nama_kawasan_pemukiman : old('nama_kawasan_pemukiman') }}"
                        {{ !isset($data) ? 'required' : '' }}>
                </div>
                <div>
                    <label for="geojson_file" class="block text-sm font-medium text-gray-900 dark:text-white mt-2 mb-1">File
                        (dengan format .geoJson)</label>
                    @isset($data)
