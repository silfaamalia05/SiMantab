    ...
    <div class="container mx-auto p-6">
        ...
        <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-4">
            <div class="flex justify-between text-center items-center dark:text-white my-2">
                <h6>{{ isset($data) ? 'Edit' : 'Tambah' }} Data Infrasturktur</h6>
                <a href="{{ route('infrastruktur.index') }}" class="bg-blue-900 text-white px-6 py-2 no-underline"><i
            ...
            @endif
            <form action="{{ isset($data) ? route('infrastruktur.update', $data->id) : route('infrastruktur.store') }}" method="POST" enctype="multipart/form-data">
                @method(isset($data) ? 'PUT' : 'POST')
                @csrf
                <div>
                    <label for="style"
                        class="block text-sm font-medium text-gray-900 dark:text-white mt-2 mb-1">Icon</label>
                    <img src="{{ asset(isset($data) && $data->style != '-' ? 'uploads/' . $data->style : 'assets/img/lokasi.png') }}"
                        class="elevation-2 mb-2" id="view_img" alt="Location Icon" width="50">
                    <input type="file" accept="image/*" name="style" id="style"
                        placeholder="Kode Infrastruktur"
                        class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                </div>
                <div>
                    <label for="kode_infrastruktur"
                        class="block text-sm font-medium text-gray-900 dark:text-white mt-2 mb-1">Kode Infrastruktur</label>
                    <input type="text" name="kode_infrastruktur" id="kode_infrastruktur" required
                        placeholder="Kode Infrastruktur"
                        class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        value="{{ isset($data) ? $data->kode_infrastruktur : old('kode_infrastruktur') }}"
                        {{ !isset($data) ? 'required' : '' }}>
                </div>
                <div>
                    <label for="nama_infrastruktur"
                        class="block text-sm font-medium text-gray-900 dark:text-white mt-2 mb-1">Nama Infrastruktur</label>
                    <input type="text" name="nama_infrastruktur" id="nama_infrastruktur" required
                        placeholder="Nama Infrastruktur"
                        class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        value="{{ isset($data) ? $data->nama_infrastruktur : old('nama_infrastruktur') }}"
                        {{ !isset($data) ? 'required' : '' }}>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="properties_show" id="properties_show" required
                        placeholder="Properties Show"
                        class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
