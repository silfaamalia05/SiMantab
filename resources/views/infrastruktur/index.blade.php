...
@section('content')
    <div class="container mx-auto p-6">
        <h2 class="text-xl font-semibold mb-4 dark:text-white">Data Infrastruktur</h2>
        <div class="bg-white dark:bg-gray-800 dark:text-white shadow-md rounded-lg p-4">
            <div class="flex justify-between text-center items-center my-2">
                <h6>Data Infrasturktur</h6>
                <a href="{{ route('infrastruktur.create') }}" class="bg-blue-900 text-white px-6 py-2 no-underline"><i
            ...
            <table class="min-w-full bg-white dark:bg-gray-900 border border-gray-200 rounded-lg">
                <thead>
                    <tr class="bg-gray-100 dark:bg-gray-900">
                        ...
                        <th class="py-2 px-4 border-b text-left">Nama Infrasturktur</th>
                ...
                <tbody>
                    @forelse ($data as $item)
                        <tr class="border-b">
                            ...
                            <td class="py-2 px-4 border-b text-left">{{ $item->kode_infrastruktur }}</td>
                            <td class="py-2 px-4 border-b text-left">{{ $item->nama_infrastruktur }}</td>
                            <td class="py-2 px-4 text-left">
                                <a
                                    href="{{ asset('uploads/geojson/' . $item->geojson_file) }}">{{ $item->geojson_file }}</a>
                            </td>
                            <td class="py-2 px-4 text-left">
                                <img src="{{asset( $item->style != '-' ? 'uploads/'.$item->style : 'assets/img/lokasi.png')}}" class="elevation-2" alt="User Image" width="50">
                            </td>
                            ...
                            <td class="py-2 px-4 text-center">
                                <form action="{{ route('infrastruktur.destroy', $item->id) }}" method="POST">
                                    @method('DELETE')
                                    @csrf
                                    <a href="{{ route('infrastruktur.edit', $item->id) }}"
