...
@section('content')
    <div class="container mx-auto p-6">
        <h2 class="text-xl font-semibold mb-4 dark:text-white">Data Tabulasi Bahaya</h2>
        <div class="bg-white shadow-md rounded-lg p-4">
            <div class="flex justify-between text-center items-center my-2">
                <h6>Data Tabulasi Bahaya</h6>
                    ...
                    @forelse ($data as $item)
                        <tr>
                            ...
                            <td class="py-2 px-4 border-b text-left">
                                {{ $item->TabulasiBahayaBanjir->lb_rendah ?? '0' }}</td>
                            <td class="py-2 px-4 border-b text-left">
                                {{ $item->TabulasiBahayaBanjir->lb_sedang ?? '0' }}</td>
                            <td class="py-2 px-4 border-b text-left">
                                {{ $item->TabulasiBahayaBanjir->lb_tinggi ?? '0' }}</td>
                            <td class="py-2 px-4 border-b text-left">
                                @php
                                    $total = 0;
                                    $total += $item->TabulasiBahayaBanjir->lb_rendah ?? 0;
                                    $total += $item->TabulasiBahayaBanjir->lb_sedang ?? 0;
                                    $total += $item->TabulasiBahayaBanjir->lb_tinggi ?? 0;
                                    echo number_format($total);
                                @endphp
                            </td>
                            <td class="py-2 px-4 border-b text-left">{{ $item->TabulasiBahayaBanjir->kelas  ?? 'Belum diset' }}</td>
                            <td class="py-2 px-4 border-b text-center"> <a
                                    href="{{ route('tabulasi_bahaya.form', $item->id) }}"
